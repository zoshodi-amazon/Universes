...
in
{
  ...
  config.flake.modules.homeManager.ssh = lib.mkIf ssh.enable {
    programs.ssh = {
      ...
      matchBlocks = { "*" = { compression = ssh.compression; serverAliveInterval = ssh.serverAliveInterval; serverAliveCountMax = ssh.serverAliveCountMax; forwardAgent = ssh.forwardAgent; }; } // lib.mapAttrs hostToMatch hosts;
    };
  };
}
