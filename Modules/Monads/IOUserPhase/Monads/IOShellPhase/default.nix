...
let
  ...
  shell = cfg.shell; env = shell.env; aliases = shell.aliases;
  zsh = shell.zsh; fish = shell.fish; nu = shell.nushell; direnv = shell.direnv;
in
{
  config.flake.modules.homeManager.shell = {
    home.sessionVariables = { EDITOR = env.EDITOR; VISUAL = env.VISUAL; KEYTIMEOUT = toString env.KEYTIMEOUT; };
    programs.starship.enable = true;
  };
  config.flake.modules.homeManager.zsh = lib.mkIf zsh.enable {
    programs.zsh = { enable = true; shellAliases = aliases;
      initContent = ''
  ...
  config.flake.modules.homeManager.fish = lib.mkIf fish.enable {
    programs.fish = { enable = true; shellAliases = aliases;
  ...
  config.flake.modules.homeManager.nushell = lib.mkIf nu.enable {
    programs.nushell = { enable = true;
      ...
      envFile.text = "$env.PATH = ($env.PATH | split row (char esep) | prepend [${lib.concatStringsSep ", " (map (p: "\"${p}\"") nu.paths)}])\n${nu.envExtra}";
    };
    programs.starship.enableNushellIntegration = true;
  };
  config.flake.modules.homeManager.direnv = lib.mkIf direnv.enable {
    programs.direnv = { enable = true; nix-direnv.enable = direnv.nixDirenv; };
  };
}
