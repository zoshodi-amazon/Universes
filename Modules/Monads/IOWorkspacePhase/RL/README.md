...
| Pipeline | PipelineInput | PipelineOutput | IOMainPhase | `pipeline` |
...
│       └── OptimizeOutput/
├── Env/
│   ├── Inputs/              # [Free] Ephemeral runtime reads
│   │   ├── cache/           # yfinance download cache
│   │   └── .env             # Broker API keys (gitignored)
│   └── Outputs/             # [Forgetful] Persisted phase artifacts
...
- API keys stored in `Env/Inputs/.env` (gitignored), loaded via pydantic-settings
- Setup: copy `Env/Inputs/.env.example` → `Env/Inputs/.env`, fill in Alpaca keys
...
| ID | Question | Source | Type |
|----|----------|--------|------|
| X1 | What position is held? | ServeOutput.position_taken | stored |
...
| X4 | Is the model fresh? | pre-condition gate in IOServePhase (derived from model mtime) | derived |
