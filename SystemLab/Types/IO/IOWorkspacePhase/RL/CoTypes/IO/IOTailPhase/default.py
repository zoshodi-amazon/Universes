...
def run(cfg: TailCoHom) -> TailCoProductOutput:
    ...
    meta = TailCoProductMeta(
        trace=TraceComonad(observer_id=observer_id)
    )
    events_received = 0
    events_displayed = 0
    ...
    while not _SHUTDOWN:
        req = urllib.request.Request(server_url, headers={"Accept": "text/event-stream"})
        try:
            ...
            if not _SHUTDOWN:
                print(f"[tail] connection failed: {e} — retrying in 2s", file=sys.stderr, flush=True)
                import time
                time.sleep(2)
            ...
            meta.connection_failures += 1
            if not _SHUTDOWN:
                print(f"[tail] error: {e} — retrying in 2s", file=sys.stderr, flush=True)
                import time
                time.sleep(2)
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        json_file="CoTypes/IO/IOTailPhase/default.json",
        json_file_encoding="utf-8",
        cli_parse_args=True,
        cli_prog_name="tail",
    )
    tail: TailCoHom = Field(default_factory=TailCoHom, description="Tail observer config — port, hostname, filter_kinds")
