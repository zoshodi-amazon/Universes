...
def run(cfg: VisualizeCoHom) -> VisualizeCoProductOutput:
    ...
    for row in rows:
        ...
        for phase_key, duration_s in timing.items():
            rr.log(f"/pipeline/{run_id}/timing/{phase_key}/duration_s", rr.Scalar(float(duration_s)))
            n_series += 1
        ...
        for i, err in enumerate(errors):
            ...
            rr.log(f"/pipeline/{run_id}/errors/{phase}/{i}/message", rr.TextLog(msg, level=rr.TextLogLevel.ERROR))
            n_errors += 1
            n_series += 1
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        json_file="CoTypes/IO/IOVisualizePhase/default.json",
        json_file_encoding="utf-8",
        cli_parse_args=True,
        cli_prog_name="visualize",
    )
