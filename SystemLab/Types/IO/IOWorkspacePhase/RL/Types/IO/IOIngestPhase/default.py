...
def run(specs: IngestHom, asset: AssetIdentity, run_base: RunIdentity) -> IngestProductOutput:
    started = datetime.now(timezone.utc).isoformat()
    meta = IngestProductMeta()
    meta.obs.started_at = started
    meta.obs.phase = PhaseId.ingest

    store = run_base.store.model_copy(update={"run_id": run_base.run_id, "phase": PhaseId.ingest})
    ...
    if len(df) > 1:
        ...
        meta.data_gaps_count = int((time_diffs > expected_gap * 2).sum())
