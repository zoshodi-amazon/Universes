...
class Settings(BaseSettings):
    ...
    env: EnvDependent = Field(default_factory=EnvDependent, description="Trading environment")
    risk: RiskDependent = Field(default_factory=RiskDependent, description="Risk gate — stop-loss and take-profit thresholds")
    discovery: DiscoveryHom = Field(default_factory=lambda: DiscoveryHom(io_universe=[]), description="Discovery config")
    ingest: IngestHom = Field(default_factory=IngestHom, description="Ingest config")
    feature: FeatureHom = Field(default_factory=FeatureHom, description="Feature config")
    train: TrainHom = Field(default_factory=TrainHom, description="Train config")
    eval: EvalHom = Field(default_factory=EvalHom, description="Eval config")
    main: MainHom = Field(default_factory=MainHom, description="Main phase config — walk-forward + optimization")
...
def _run_pipeline(settings: Settings, trial_train: TrainHom | None = None) -> MainProductOutput:
    ...
    train_cfg = trial_train if trial_train else settings.train

    np.random.seed(settings.run.seed)
    torch.manual_seed(settings.run.seed)

    try:
        discovery_record = discover(settings.discovery, settings.asset, settings.run)
        ...
        return MainProductOutput(
            ...
            duration_s=0.0, status=MainStatus.failed, meta=meta,
        )
    ...
    feat_row = settings.run.store.model_copy(
    ...
    for i in range(n_windows):
        ...
        try:
            train_record = train(feature_record, train_cfg, settings.env, asset, settings.run, train_slice)
            eval_record = evaluate(train_record, settings.eval, settings.env, settings.risk, asset, settings.run, i, eval_slice)
...
def _run_optimize(settings: Settings) -> MainProductOutput:
    ...
    store = settings.run.store.model_copy(update={"run_id": settings.run.run_id, "phase": "pipeline"})
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        trial_settings = Settings(
            asset=settings.asset, run=trial_run, env=settings.env, risk=settings.risk,
            discovery=settings.discovery, ingest=settings.ingest, feature=settings.feature,
            train=trial_train, eval=settings.eval, main=settings.main.model_copy(update={"optimize": False}),
        )
...
def _write(record: MainProductOutput, run_base: RunIdentity) -> None:
    store = run_base.store.model_copy(update={"run_id": run_base.run_id, "phase": "pipeline"})
