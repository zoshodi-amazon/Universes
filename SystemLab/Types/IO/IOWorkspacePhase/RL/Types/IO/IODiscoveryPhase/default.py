...
INTERVAL_MAP: dict[int, str] = {1: "1m", 5: "5m", 15: "15m", 30: "30m", 60: "1h", 1440: "1d"}
...
class Settings(BaseSettings):
    ...
    run: RunIdentity = Field(default_factory=RunIdentity, description="Run context — ID, seed, store")
    discovery: DiscoveryHom = Field(default_factory=lambda: DiscoveryHom(io_universe=[]), description="Discovery config — screener, ADX threshold, universe")
...
def _route_by_asset_class(tickers: list[str], asset_type: AssetType) -> list[str]:
    ...
    if asset_type == AssetType.crypto:
        ...
        return [t for t in tickers if "-USD" not in t and "=X" not in t and "-USDT" not in t]
...
def _filter_by_liquidity(
    ...
    for ticker in tickers:
        try:
            ...
            price: float = info.regular_market_price if info.regular_market_price >= 0 else 0.0
            day_high: float = info.day_high if info.day_high >= 0 else price
            day_low: float = info.day_low if info.day_low >= 0 else price
            atr_pct: float = ((day_high - day_low) / price * 100) if price > 0 else 100.0
            ticker_data.append((ticker, avg_vol, price, atr_pct))
        except Exception:
            continue
    ...
    vol_threshold: float = float(np.percentile(volumes, 100 - specs.liquidity.min_volume_percentile))
    price_threshold: float = float(np.percentile(prices, specs.liquidity.min_price_percentile))

    filtered: list[str] = []
    for ticker, vol, price, atr_pct in ticker_data:
...
def _evaluate_alarms(meta: DiscoveryProductMeta, specs: DiscoveryHom, n_qualifying: int) -> None:
    """Evaluate alarm thresholds and emit alarms."""
    if not specs.alarms.enabled:
        return

    if n_qualifying < specs.alarms.min_qualifying_tickers:
        meta.obs.alarms.append(AlarmMonad(
    ...
    api_failures: int = len([e for e in meta.obs.errors if "failed" in e.message.lower()])
...
def run(specs: DiscoveryHom, asset: AssetIdentity, run_base: RunIdentity) -> DiscoveryProductOutput:
    ...
    store = run_base.store.model_copy(update={"run_id": run_base.run_id, "phase": PhaseId.discovery})
    ...
    for ticker in universe:
        ...
        try:
            if cache_path.exists():
                ...
                raw_df: pd.DataFrame = yf.download(ticker, period=specs.adx_lookback_period, interval=yf_interval, auto_adjust=True)
    ...
    meta.obs.metrics.extend([
        MetricMonad(name="screener_result_count", value=float(meta.screener_result_count)),
        MetricMonad(name="asset_class_filtered", value=float(meta.asset_class_filtered_count)),
        MetricMonad(name="liquidity_filtered", value=float(meta.liquidity_filtered_count)),
