...
class Settings(BaseSettings):
    ...
    eval: EvalHom = Field(
        default=EvalHom(), description="Eval config — forward window, profit threshold"
    )
...
if __name__ == "__main__":
    ...
    train_record = train(
        feature_record, train_cfg, s.env, s.asset, s.run, train_slice, s.store
    )
    run(train_record, s.eval, s.env, s.risk, s.asset, s.run, 0, eval_slice, s.store)
