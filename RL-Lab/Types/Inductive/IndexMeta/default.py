...
class IndexMetaInductive(BaseModel):
    ...
    average_volume: float = Field(
        default=-1.0, ge=-1.0, description="Average daily volume (-1.0 if unavailable)"
    )
    regular_market_price: float = Field(
        default=-1.0, ge=-1.0, description="Current market price (-1.0 if unavailable)"
    )
    day_high: float = Field(
        default=-1.0, ge=-1.0, description="Day high price (-1.0 if unavailable)"
    )
    day_low: float = Field(
        default=-1.0, ge=-1.0, description="Day low price (-1.0 if unavailable)"
    )
    market_cap: float = Field(
        default=-1.0, ge=-1.0, description="Market capitalization (-1.0 if unavailable)"
    )
    ...
    def from_info(
        ...
        return cls(
            ...
            regular_market_price=float(
                info.get("regularMarketPrice", -1)
                or info.get("previousClose", -1)
                or -1
            ),
