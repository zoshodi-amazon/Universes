"""MeasureMonad [Monad] — Counter or gauge measurement (3 fields).
...
class MeasureMonad(BaseModel):
    """MeasureMonad [Monad] — Single metric observation point."""

    name: Annotated[
        str,
        StringConstraints(pattern=r"^[a-z_][a-z0-9_]*$", min_length=1, max_length=64),
    ] = Field(..., description="Metric name in snake_case")
    value: float = Field(ge=-1e15, le=1e15, description="Metric value")
    kind: MeasureInductive = Field(
        default=MeasureInductive.gauge,
