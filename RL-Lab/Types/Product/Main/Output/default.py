"""MainProductOutput [Product] — Full pipeline output (7 params). All bounded.

Returns in percentage. Composes EvalProductOutput list and MainProductMeta (errors + optimization results).
...
class MainProductOutput(BaseModel):
    """MainProductOutput [Product] — Result of full pipeline: Discovery -> Ingest -> Feature -> (Train -> Eval)*."""
    run_id: Annotated[str, StringConstraints(pattern=r"^[a-f0-9]{8}$", min_length=8, max_length=8)] = Field(default_factory=lambda: uuid.uuid4().hex[:8], description="8-char hex run identifier")
    ...
    status: MainStatus = Field(default=MainStatus.success, description="Pipeline outcome — success, partial, or failed")
    results: list[EvalProductOutput] = Field(default_factory=list, max_length=10_000, description="Per-window evaluation results")
    meta: MainProductMeta = Field(default_factory=MainProductMeta, description="Pipeline metadata: errors + optimization results")
