...
let cfg = config.git; in
{
  config.flake.modules.homeManager.git = lib.mkIf cfg.enable {
    programs.git = {
      ...
      settings = { init.defaultBranch = cfg.defaultBranch; alias = cfg.aliases; } // cfg.extraConfig;
      lfs.enable = cfg.lfs.enable;
      ignores = cfg.ignores;
    };
    programs.delta = { enable = cfg.delta.enable; enableGitIntegration = true; };
  };
}
