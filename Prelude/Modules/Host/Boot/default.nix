# Boot — global instantiation
# Wires NixBoot + NixDisk Artifacts into flake.modules.nixos
{ config, lib, inputs, ... }:
let
  cfg = config.boot-config;
  mkPartContent = part:
    if part.type == "efi" then { type = "filesystem"; format = "vfat"; mountpoint = if part.mountpoint != null then part.mountpoint else "/boot"; }
    else if part.type == "swap" then { type = "swap"; randomEncryption = true; }
    else { type = "filesystem"; format = part.filesystem; mountpoint = part.mountpoint; };
  mkDiskoDevice = _: dev: {
    type = "disk"; device = dev.device;
    content = { type = dev.tableType; partitions = lib.listToAttrs (map (part: {
      name = part.name; value = { size = part.size; content = mkPartContent part; };
    }) dev.partitions); };
  };
in
{
