...
def _make_env(df: pd.DataFrame, env_base: EnvDependent, episode_bars: int, verbose: int) -> Callable[[], gym.Env]:
    def _init() -> gym.Env:
        return gym.make(
...
def run(
    ...
    store = run_base.store.model_copy(update={"run_id": run_base.run_id, "phase": PhaseId.train})
...
class Settings(BaseSettings):
    ...
    train: TrainHom = Field(default=TrainHom(), description="Train config — algorithm, timesteps, learning rate, envs")
