...
class VisualizeCoProductMeta(BaseModel):
    ...
    run_ids_found: list[Annotated[str, StringConstraints(min_length=1, max_length=32)]] = Field(
        default_factory=list, max_length=10_000,
        description="Distinct run_ids found in scanned ProductOutput JSON files")
    phases_found: list[Annotated[str, StringConstraints(min_length=1, max_length=32)]] = Field(
        default_factory=list, max_length=20,
        description="Phase names (discovery, ingest, ...) for which artifacts were found and logged")
    feature_columns_logged: int = Field(default=0, ge=0, le=10_000,
