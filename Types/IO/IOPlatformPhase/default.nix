...
in
{
  config.flake.modules.nixos.boot-config =
    {
      pkgs,
      lib,
      config,
  ...
  config.flake.modules.nixos.disk-config = lib.mkIf disk.enable (
    { pkgs, ... }:
    {
      ...
      boot.initrd.network = lib.mkIf disk.remoteUnlock {
        enable = true;
        ssh = {
          enable = true;
          port = 2222;
  ...
  config.flake.modules.nixos.hardware-config = lib.mkIf hw.enable (
    {
      pkgs,
      lib,
      config,
    ...
    }:
    {
      ...
      config = lib.mkIf config.hardware-config.enable {
        ...
        services.pipewire = lib.mkIf (hw.audio == "pipewire") {
          enable = true;
          alsa.enable = true;
          pulse.enable = true;
        };
