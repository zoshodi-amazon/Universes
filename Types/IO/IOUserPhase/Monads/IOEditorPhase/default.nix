...
in
{
  config.flake.modules.homeManager.nixvim = lib.mkIf nvim.enable {
    ...
    programs.nixvim = { pkgs, ... }: {
      ...
      opts = { number = nvim.lineNumbers; relativenumber = nvim.relativeNumbers;
        tabstop = nvim.tabWidth; shiftwidth = nvim.tabWidth; expandtab = true; smartindent = true;
      ...
      plugins = {
        ...
        lsp = { enable = true; servers = {
          nil_ls.enable = true; nushell.enable = true; pyright.enable = true;
          rust_analyzer = { enable = true; installCargo = false; installRustc = false; };
          ts_ls.enable = true; lua_ls.enable = true; taplo.enable = true;
        }; };
        none-ls = { enable = true; sources.formatting.alejandra.enable = true; };
        telescope = { enable = true;
          settings.defaults.vimgrep_arguments = [ "rg" "--color=never" "--no-heading" "--with-filename" "--line-number" "--column" "--smart-case" "--hidden" "--glob" "!.git/*" ];
          settings.pickers.find_files.find_command = [ "rg" "--files" "--hidden" "--glob" "!.git/*" ];
        };
        ...
        gitsigns.enable = true; fugitive.enable = true;
        markdown-preview = { enable = true; autoLoad = true; };
      };
      extraPlugins = [
        { plugin = pkgs.vimUtils.buildVimPlugin { name = "d2"; src = pkgs.fetchFromGitHub { owner = "terrastruct"; repo = "d2-vim"; rev = "master"; sha256 = "0c6sg882mb6za9zgv83h1jcc9q9y0ppfqpm4q9vmyj98w9yd0q0y"; }; };
          config = "let g:d2_fmt_autosave = 1\nlet g:d2_ascii_autorender = 1\nlet g:d2_ascii_preview_width = 80"; }
        { plugin = pkgs.vimUtils.buildVimPlugin { name = "live-server"; src = pkgs.fetchFromGitHub { owner = "barrett-ruth"; repo = "live-server.nvim"; rev = "main"; sha256 = "0hfgcz01l38arz51szbcn9068zlsnf4wsh7f9js0jfw3r140gw6h"; }; };
          config = ""; }
      ];
      keymaps = [
        { mode = "i"; key = "jk"; action = "<Esc>"; options.desc = "Exit insert mode"; }
        { mode = "n"; key = "<C-h>"; action = "<C-w>h"; options.desc = "Left window"; }
        { mode = "n"; key = "<C-j>"; action = "<C-w>j"; options.desc = "Bottom window"; }
        { mode = "n"; key = "<C-k>"; action = "<C-w>k"; options.desc = "Top window"; }
        ...
        { mode = "n"; key = "<leader>cc"; action = "<cmd>!nix flake check<cr>"; options.desc = "Nix check"; }
        ...
        { mode = "n"; key = "<leader>ib"; action = "<cmd>Telescope buffers<cr>"; options.desc = "Buffers"; }
        { mode = "n"; key = "<leader>mh"; action = "<cmd>Telescope help_tags<cr>"; options.desc = "Help"; }
        { mode = "n"; key = "<leader>mk"; action = "<cmd>Telescope keymaps<cr>"; options.desc = "Keymaps"; }
        { mode = "n"; key = "<leader>e"; action = "<cmd>Oil<cr>"; options.desc = "File explorer"; }
        { mode = "n"; key = "<leader>it"; action = "<cmd>NvimTreeToggle<cr>"; options.desc = "File tree"; }
        ...
        { mode = "n"; key = "<leader>lr"; action = "<cmd>terminal<cr>"; options.desc = "Terminal"; }
        { mode = "n"; key = "<leader>io"; action = "<cmd>!open %<cr>"; options.desc = "Open in app"; }
        ...
        { mode = "v"; key = "<leader>dj"; action = ":'<,'>!jq .<cr>"; options.desc = "Format selection"; }
        ...
        { mode = "n"; key = "<leader>rs"; action = "<cmd>PreviewSend<cr>"; options.desc = "Render send"; }
        { mode = "n"; key = "<leader>rm"; action = "<cmd>MarkdownPreview<cr>"; options.desc = "Markdown preview"; }
        { mode = "n"; key = "<leader>rl"; action = "<cmd>LiveServerStart<cr>"; options.desc = "Live server"; }
